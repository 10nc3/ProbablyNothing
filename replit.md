...
- **Present**: 222 tests passing. Pipeline hardened (audit metrics, source tagging, input guards, error chain, context truncation, PII anonymization, log rotation at 1000 entries). Strike system restored (3-strike demotion, 5-min cooldown, passive health stats).
...
- **Exec Watchtower**: `lib/exec-watchtower.js` — safe process execution satellite for prescribe mode. Foreground (sync) + background (async with polling) modes. child_process.spawn with configurable timeout (30s FG, 120s BG), output truncation (4KB cap), SIGTERM→SIGKILL grace. Background process registry (Map, 20 max, LRU eviction). Safety: reuses void-pipeline's 33+ dangerous pattern guard + path traversal check. ENV hardening blocks LD_PRELOAD/LD_LIBRARY_PATH/DYLD_*/PATH overrides. Minimal PATH: /usr/local/bin:/usr/bin:/bin + workspace node_modules/.bin. Synthesized from openclaw/openclaw PR #14257 PTY supervisor patterns — cut PTY/container/send-keys bloat, kept safety core.
- **Swarm Coordinator**: `lib/swarm-coordinator.js` — parallel sub-agent orchestration satellite. Fan-out: decomposes tasks into workers, each runs through void-pipeline with isolated sessionId (parent:swarm:N). Fan-in: aggregates responses with token budget tracking (default 50K tokens, aborts remaining workers on budget exceed). Registry: Map, 10 workers/swarm, 5 concurrent swarms, LRU eviction. Lifecycle: pending→running→done|failed|aborted. Memory cleanup on completion. Synthesized from openclaw/openclaw subagent-registry patterns — cut WebSocket/CLI/node-pairing bloat, kept orchestration core.
- **API Endpoints**: `/api/exec` (POST, trusted-IP only) foreground/background exec. `/api/exec/status` (GET) poll/list. `/api/exec/stop` (POST) kill. `/api/swarm` (POST, trusted-IP only) run parallel tasks. `/api/swarm/status` (GET) inspect. `/api/swarm/abort` (POST) abort. Health endpoint includes satellite status.
...
- **Skip from openclaw**: Cron webhooks, CLI pairing, PTY supervisor (send-keys/paste), sub-agent nesting, container sandboxing, apply_patch subtool, multi-profile isolation — platform-specific or not needed yet.