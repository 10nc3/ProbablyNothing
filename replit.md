...
- 2026-02-14: Synthesis round 4 — nyanclaw commit ed9de1f reviewed. Absorbed 3 improvements into formatPsiEMA: (1) REVERSAL reading (theta>0, R<0.618 = momentum reversal), (2) always show dual daily+weekly (no conditional), (3) N/A guard for missing theta data. Confirmed 6 nyanclaw bugs: dead context-router import in pipeline (crash), llm-client untouched (3 providers, no system prompt, no clamping), PRIVILEGED_CALLER_ID regression (hardcoded phone fallback), two competing formatPsiEMA versions, barrel export incomplete, detectIntents collision (pipeline vs intent-detector never merged). 142 tests passing.
