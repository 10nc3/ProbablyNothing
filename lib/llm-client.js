...
async function callWithFallback(prompt, options = {}) {
  ...
  if (options.provider) {
    ...
    try {
      ...
      recordSuccess(options.provider, Date.now() - t0);
      return result;
    } catch (e) {
      recordStrike(options.provider, e.message);
      throw e;
    }
  }
  ...
  const chainSummary = errors.map(e => `${e.provider}: ${e.error}`).join(' | ');
  throw new Error(`All ${errors.length} providers failed — ${chainSummary}`);
}
