...
function printBanner(env, port) {
  ...
  lines.push(`  ${C.dim}(cloud providers verified passively from real requests)${C.reset}`);
  ...
  if (env.ollama.available) {
    ...
    lines.push(`  ${ok(`ollama  ${C.dim}${env.ollama.url} (confirmed)${C.reset}`)}`);
  ...
  for (const p of cloudProviders) {
    ...
    if (status.configured) {
      lines.push(`  ${ok(`${p.label} ${C.dim}(${p.envKey} configured)${C.reset}`)}`);
    } else {
  ...
  const privIds = (process.env.PRIVILEGED_CALLER_ID || '').split(',').map(s => s.trim()).filter(Boolean);
  if (privIds.length > 0) {
    const masked = privIds.map(id => id.length > 6 ? id.slice(0, 3) + '***' + id.slice(-3) : '***');
    lines.push(`  ${C.magenta}prescribe${C.reset} ${C.dim}build/kernel  ${C.reset}${C.green}[${masked.join(', ')}]${C.reset}`);
  } else {
    lines.push(`  ${C.magenta}prescribe${C.reset} ${C.dim}build/kernel  ${C.reset}${C.yellow}[locked — set PRIVILEGED_CALLER_ID]${C.reset}`);
  }
  ...
  if (env.ollama.available) {
    lines.push('');
    lines.push(`  ${C.green}Substrate: local Ollama (always available as last resort)${C.reset}`);
  }

  lines.push('');
  lines.push('');
  lines.push(`${C.bold}  CHANNELS${C.reset}`);
  lines.push(`  ${C.white}http${C.reset}     ${C.dim}/api/chat (always available)${C.reset}`);
  const hasDiscordToken = !!process.env.DISCORD_BOT_TOKEN;
  if (hasDiscordToken) {
    lines.push(`  ${ok('discord  event gateway (bot token set)')}`);
  } else {
    lines.push(`  ${fail('discord  set DISCORD_BOT_TOKEN to enable')}`);
  }
