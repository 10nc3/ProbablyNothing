...
async function runPipeline(input) {
  const { query, sessionId = 'default', callerId = null, options = {}, chain = null, image = null, imageUrl = null, imageMime = null } = input;
  const t0 = Date.now();
  ...
  const hasVision = !!(image || imageUrl);
  const shouldUseNyan = hasNyanIntent || complexity === 'heavy' || hasVision;

  if (!response && shouldUseNyan) {
    try {
      const visionOpts = hasVision ? { image, imageUrl, imageMime } : null;
      const domain = hasVision ? 'vision' : (complexity === 'heavy' ? 'reasoning' : undefined);
      const result = await atomicQuery(query, domain, visionOpts);
