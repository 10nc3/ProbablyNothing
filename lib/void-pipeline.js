...
function formatPsiEMA(stockData) {
  ...
  function formatOne(data, tickerKey) {
    ...
    const lines = [
      ...
      `Daily:  ${reading} (theta: ${theta}deg | z: ${z}s | R: ${r})`,
    ];
    ...
    if (w.theta !== undefined) {
      ...
      const wReading = getReading(wTheta, wZ, wR);
      lines.push(`Weekly: ${wReading} (theta: ${wTheta}deg | z: ${wZ}s | R: ${wR})`);
    } else {
      lines.push(`Weekly: NO DATA`);
    }
