...
const COMPACTION_TIMEOUT_MS = 30000;
...
class LocalMemoryManager {
  ...
  async generateSummary() {
    ...
    if (this._compacting) {
      console.warn('[memory] compaction already in progress — skipping');
      return this.currentSummary;
    }
...
Write the summary as natural prose, not a list. Focus on what a helpful assistant would need to remember to continue this conversation naturally.`;
    ...
    try {
      const result = await Promise.race([
        ...
        new Promise((_, reject) => {
          const timer = setTimeout(() => reject(new Error('compaction timeout')), COMPACTION_TIMEOUT_MS);
          signal.addEventListener('abort', () => {
            clearTimeout(timer);
            reject(new Error('compaction aborted — session reset'));
          });
        })
      ]);

      if (signal.aborted) return this.currentSummary;

      this.currentSummary = (result || '').trim();
      ...
      if (error.message.includes('compaction aborted')) {
        console.log('[memory] compaction aborted cleanly — session was reset');
      } else if (error.message.includes('compaction timeout')) {
        console.warn(`[memory] compaction timed out after ${COMPACTION_TIMEOUT_MS}ms — preserving existing summary`);
      } else {
  ...
  clear() {
    if (this._abortController) {
      ...
      console.log('[memory] aborted in-flight compaction during session reset');
    }
    this.messages = [];
    this.currentSummary = "";
    this.attachments = [];
...
function _evictOldestSession() {
  if (sessionMemories.size <= MAX_SESSIONS) return;
  let oldestKey = null;
  let oldestTime = Infinity;
  for (const [key, manager] of sessionMemories) {
    const lastActivity = Math.max(
      manager.lastSummaryTime || 0,
      ...manager.messages.map(m => m.timestamp || 0),
      0
    );
    const t = lastActivity > 0 ? lastActivity : 0;
    if (t < oldestTime) {
      oldestTime = t;
      oldestKey = key;
    }
  ...
  if (oldestKey) {
    sessionMemories.get(oldestKey)?.clear();
...
setInterval(() => {
  ...
  if (cleaned > 0) {
    console.log(`[memory] auto-cleaned ${cleaned} expired sessions (TTL: ${SESSION_TTL_MS / 60000} min)`);
  }
}, 5 * 60 * 1000);
