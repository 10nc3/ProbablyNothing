 ...
 * @param {Array<string>} options.photos - Array of base64 image data
 * @param {Array<object>} options.documents - Array of {name, data, type}
...
/**
 * Call Ψ-EMA via dedicated /psi-ema endpoint (LLM-free calculation)
 * @param {string|string[]} ticker - Stock ticker or array of tickers
 * @returns {Promise<object>} Ψ-EMA data with results keyed by ticker
 */
async function getPsiEMA(ticker) {
  ...
  try {
    ...
    console.log(`[nyan-api] psi-ema endpoint failed: ${e.message}, falling back to atomic`);
    const fallback = await callNyanAPI(`$${isArray ? ticker[0] : ticker} psi-ema`);
    if (fallback.success) {
      return {
        ticker: fallback.ticker || ticker,
        psiEma: fallback.psiEma,
        psi_ema_daily: fallback.psi_ema_daily,
        psi_ema_weekly: fallback.psi_ema_weekly,
    ...
    return { error: fallback.error || e.message, ticker, success: false };
  }
}
 ...
 * @param {string} domain - Domain hint (psi-ema, legal, chemistry, reasoning, etc.)
 * @param {object} mediaOpts - Optional media data { photos, documents }
 * @returns {Promise<object>} API response
 */
async function atomicQuery(message, domain = null, mediaOpts = null) {
  ...
  return await callNyanAPI(fullMessage, mediaOpts || {});
}
