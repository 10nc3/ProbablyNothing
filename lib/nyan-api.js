 ...
 * @param {string} options.image - Base64-encoded image data (for vision queries)
 * @param {string} options.imageUrl - URL of image (for vision queries)
 * @param {string} options.imageMime - MIME type of image (e.g., 'image/png')
 * @returns {Promise<object>} API response
 */
async function callNyanAPI(message, options = {}) {
  const { timeout = 30000, image, imageUrl, imageMime } = options;
  ...
  if (image || imageUrl) {
    payload.vision = {};
    if (image) {
      payload.vision.image = image;
      payload.vision.mime = imageMime || 'image/png';
    }
    if (imageUrl) {
      payload.vision.url = imageUrl;
    }
  }
 ...
 * @param {object} visionOpts - Optional vision data { image, imageUrl, imageMime }
 * @returns {Promise<object>} API response
 */
async function atomicQuery(message, domain = null, visionOpts = null) {
  const fullMessage = domain 
    ? `${message} [${domain}]` 
    : message;
  
  return await callNyanAPI(fullMessage, visionOpts || {});
}
