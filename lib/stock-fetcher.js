 ...
 * Routes through callWithFallback — works on Ollama, Groq, or any available provider
...
async function extractTickerWithAI(query) {
  ...
  try {
    ...
    const systemPrompt = `You are a stock ticker extraction assistant. Extract the stock ticker symbol from the user's query.
...
- "what's the weather" → {"ticker": null, "confidence": "high", "reason": "no company mentioned"}`;
    ...
    const content = await callWithFallback(query, {
      system: systemPrompt,
      temperature: 0,
      maxTokens: 100,
      timeout: 15000
